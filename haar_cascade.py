...
def connect_rtsp_stream(rtsp_url):
    "Hàm này dùng để kết nối với IP Camera thông qua giao thức RTSP"
    cap = cv2.VideoCapture(rtsp_url, cv2.CAP_FFMPEG)
    if not cap.isOpened():
        raise Exception("Cannot connect to RTSP stream")
    return cap
def extract_frame(cap, resize_shape=(640, 360)):
    "Hàm này dùng để đọc cái frame được lấy từ cv2.CAP_FFMPEG ở trên"
    ret, frame = cap.read()
    if not ret:
        return None
...
def face_detection(frame):
    "Hàm này dùng thuật toán haar_rcascade để lấy ra khuôn mặt"
